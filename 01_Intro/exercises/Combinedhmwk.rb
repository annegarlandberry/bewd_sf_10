...
def create_csv(stories, which_is_it)
  CSV.open("./file.csv", "wb") do |csv|
    ...
    stories.each do |story|
        story = create_story_hash(story, which_is_it)
       csv <<"Title: #{story[:title]}. Category: #{story[:category]}. Upvotes: #{story[:upvotes]}. API Source: #{story[:provider]}."
    end
  end
end
...
def create_story_hash(story, which_is_it)
  if which_is_it == "reddit"
    {title: story["data"]["title"], upvotes: story["data"]["ups"],  category: story["data"]["subreddit"], provider: which_is_it}
