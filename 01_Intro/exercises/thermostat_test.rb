...
class Thermostat
  ...
  #class method
  def self.detect_temperature(all_temps)
    all_temps.each do |temperature|
      ...
      @temperature.get_desired_temperature #call that awesome instance method we created
    end
  end
...
Thermostat.detect_temperature(all_temps)
